...
/**
 * Utility functions for Post entity
 */
export class PostModel {
  ...
  /**
   * Factory method to create a list of Post from JSON data
   */
  static fromJsonList(jsonList: Record<string, any>[]): Post[] {
    return jsonList.map(json => this.fromJson(json))
  }

  /**
   * Convert Post to JSON format (for API requests)
   * Optionally converts camelCase to snake_case
   */
  static toJson(entity: Partial<Post>, useSnakeCase: boolean = false): Record<string, any> {
    if (!useSnakeCase) {
      return { ...entity }
    }
